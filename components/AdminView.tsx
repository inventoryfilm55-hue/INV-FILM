...
import { Trash2, Lock, Edit3, CheckCircle, RefreshCw, Link as LinkIcon, Globe, Code, Copy, ExternalLink, MessageSquare } from 'lucide-react';
      ...
      {activeTab === 'FILMS' && (
        <div className="space-y-12">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-lg font-logo font-black text-white uppercase tracking-wider">Inventory Archive</h3>
          ...
          {(isAdding || editingProjectId) && (
            <div className="bg-white/5 border border-[#84cc16]/30 p-10 mb-16 space-y-8 animate-fade-up rounded-sm">
              ...
              <div className="flex gap-4">
                <button onClick={saveProject} className="flex-grow py-5 bg-[#84cc16] text-black font-logo font-black uppercase tracking-widest hover:bg-white transition-all">Save Changes</button>
            ...
            {projects.map((p, i) => (
              <div key={p.id} className="flex items-center gap-6 p-5 bg-white/5 border border-white/5 hover:border-[#84cc16]/30 transition-all rounded-sm group">
                ...
                <div className="flex-grow">
                  ...
                  <div className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">{p.category} | {p.client}</div>
                </div>
                <div className="flex gap-4">
                  <button onClick={() => { setEditingProjectId(p.id); setProjectFormData(p); setIsAdding(false); }} className="p-3 text-neutral-500 hover:text-white transition-all"><Edit3 size={20} /></button>
                  <button onClick={() => { if(confirm('Delete project from archive?')) onUpdateProjects(projects.filter(item => item.id !== p.id)) }} className="p-3 text-neutral-500 hover:text-red-500 transition-all"><Trash2 size={20} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === 'SITE_CONTENT' && (
        <div className="max-w-4xl space-y-12">
          ...
          <div className="bg-white/5 border border-white/10 p-10 space-y-10 rounded-sm">
            <h3 className="text-xl font-logo font-black text-[#84cc16] uppercase tracking-widest border-b border-white/5 pb-6">About Page (Hero Experience)</h3>
            ...
            <div className="grid grid-cols-1 gap-8">
              ...
              <div className="space-y-4">
                <label className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest text-[#84cc16]">Background Video (YouTube URL)</label>
                <input className="w-full bg-black border border-[#84cc16]/30 p-4 text-white" value={tempContent.about.videoBg || ''} onChange={e => setTempContent({...tempContent, about: {...tempContent.about, videoBg: e.target.value}})} placeholder="https://www.youtube.com/watch?v=..." />
                <p className="text-[9px] text-neutral-500 uppercase tracking-widest">영상 주소를 넣으면 About 페이지 배경에서 자동 재생됩니다. (비어있으면 아래 사진 1이 적용)</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Hero Image 1 (Fallback BG)</label>
                  <input className="w-full bg-black border border-white/10 p-4 text-white" value={tempContent.about.img1} onChange={e => setTempContent({...tempContent, about: {...tempContent.about, img1: e.target.value}})} />
                </div>
                <div className="space-y-4">
                  <label className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Narrative Image 2</label>
          ...
          <div className="bg-white/5 border border-white/10 p-10 space-y-10 rounded-sm">
            <h3 className="text-xl font-logo font-black text-[#84cc16] uppercase tracking-widest border-b border-white/5 pb-6">Lead Director Info</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <textarea className="w-full bg-black border border-white/10 p-4 text-white h-24" value={tempContent.directors.manifesto} onChange={e => setTempContent({...tempContent, directors: {...tempContent.directors, manifesto: e.target.value}})} />
            </div>
            <button onClick={() => onUpdateContent(tempContent)} className="w-full py-6 bg-white/10 text-white font-logo font-black uppercase hover:bg-white hover:text-black transition-all">Save Director Profile</button>
          </div>
        </div>
      )}
        ...
        <div className="max-w-4xl space-y-12 animate-fade-up pb-40">
          ...
          <div className="bg-neutral-900/50 border border-white/10 p-10 rounded-sm">
            ...
            <div className="text-white font-mono text-sm bg-black p-4 border border-white/5 flex items-center justify-between">
              ...
              <button onClick={() => { navigator.clipboard.writeText("https://www.inventoryfilm.com"); setUrlCopySuccess(true); setTimeout(()=>setUrlCopySuccess(false),2000); }} className="text-[#84cc16] hover:text-white transition-colors">
          ...
          <div className="bg-neutral-900/50 border border-[#84cc16]/20 p-10 rounded-sm">
            ...
            <button onClick={handleExportCode} className="w-full py-5 bg-[#84cc16] text-black font-logo font-black uppercase flex items-center justify-center gap-3 hover:bg-white transition-all">
              {copySuccess ? <CheckCircle size={20} /> : <Copy size={20} />}
              {copySuccess ? 'Code Copied!' : 'Generate Sync Code'}
            </button>
            {productionCode && (
              <pre className="mt-6 bg-black p-6 rounded-sm text-[9px] font-mono text-neutral-500 overflow-x-auto border border-white/5 max-h-60">
                {productionCode}
              </pre>
            )}
          </div>
