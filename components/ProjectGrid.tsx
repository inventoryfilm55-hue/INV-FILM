...
const ProjectCard: React.FC<{ project: Project; index: number; onProjectClick: (project: Project) => void }> = ({ project, index, onProjectClick }) => {
  ...
  return (
    <div
      ...
      className="relative group cursor-pointer animate-fade-up flex flex-col w-full mb-8 lg:mb-0"
    ...
    >
      <div className={`relative overflow-hidden bg-neutral-900 transition-all duration-700 w-full rounded-sm ${isVertical ? 'aspect-[9/16]' : 'aspect-video'}`}>
        <img
          ...
          className="w-full h-full object-cover transition-transform duration-[1500ms] lg:group-hover:scale-105 opacity-90 lg:group-hover:opacity-100"
          loading="lazy"
        />
        
        {/* Desktop Info Overlay */}
        <div className="hidden lg:flex absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all duration-500 flex-col items-center justify-center p-12 text-center backdrop-blur-md">
          <p className="text-[#84cc16] text-[10px] font-black tracking-[0.6em] uppercase mb-4">{project.category}</p>
          <h3 className="text-white font-logo font-black text-4xl leading-[0.9] tracking-tighter uppercase mb-6 break-words px-4">{project.title}</h3>
      ...
      {/* Mobile/Tablet Info Block - Always visible for touch devices */}
